...
const fieldMap = {
  ...
  FeeSettings: ['id', 'key', 'usd_to_etb_rate', 'gateway_fee_percentage', 'checkout_preview_fee_percentage', 'total_amount_fee_percentage', 'deposit_fee_percentage', 'deposit_fixed_fee_etb', 'service_margin_percentage', 'minimum_service_fee_etb', 'maximum_service_fee_etb', 'enable_minimum_fee', 'show_gateway_fee_percentage', 'safety_buffer_percentage', 'chapa_settlement_fee_etb', 'card_creation_fee_usd', 'bitnob_topup_fee_under_100_usd', 'bitnob_topup_fee_percent_100_plus', 'card_funding_fee_percentage', 'card_withdrawal_fee_percentage', 'rounding_rule_etb', 'customer_fee_display_style', 'min_deposit_usd', 'max_deposit_usd', 'daily_deposit_limit_usd', 'monthly_deposit_limit_usd', 'min_card_creation_usd', 'min_card_funding_usd', 'max_card_funding_usd', 'max_cards_per_user', 'kyc_level1_deposit_limit', 'kyc_level2_deposit_limit', 'created_at', 'updated_at'],
...
function normalizeFeeSettings(payload) {
  return {
    ...
    usd_to_etb_rate: Number.isFinite(Number(payload.usd_to_etb_rate)) ? Number(payload.usd_to_etb_rate) : 190,
    gateway_fee_percentage: Number.isFinite(Number(payload.gateway_fee_percentage)) ? Number(payload.gateway_fee_percentage) : 2.88,
    checkout_preview_fee_percentage: Number.isFinite(Number(payload.checkout_preview_fee_percentage)) ? Math.max(0, Number(payload.checkout_preview_fee_percentage)) : 2.5,
    total_amount_fee_percentage: Number.isFinite(Number(payload.total_amount_fee_percentage)) ? Math.max(0, Number(payload.total_amount_fee_percentage)) : 2.5,
    deposit_fee_percentage: 0,
    deposit_fixed_fee_etb: 0,
    service_margin_percentage: Number.isFinite(Number(payload.service_margin_percentage)) ? Number(payload.service_margin_percentage) : 5,
    minimum_service_fee_etb: 0,
    maximum_service_fee_etb: 0,
    enable_minimum_fee: 0,
    show_gateway_fee_percentage: payload.show_gateway_fee_percentage === undefined ? 1 : (payload.show_gateway_fee_percentage ? 1 : 0),
